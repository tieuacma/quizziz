...
function isQuizLoaded(
    result: QuizLoadResult
): result is Extract<QuizLoadResult, { success: true }> {
...
export function useQuizEditor(
    ...
    const [history, setHistory] = useState<
        { metadata: QuizMetadata | null; questions: QuizQuestion[] }[]
    >([]);
    ...
    const pushHistoryDebounced = useCallback(
        (meta: QuizMetadata | null, quest: QuizQuestion[]) => {
                ...
                setHistory((prev) => {
                    ...
                    if (
                        ...
                        JSON.stringify(currentItem) ===
                            JSON.stringify({ metadata: meta, questions: quest })
                    ) {
                        return prev;
                    }
                    const nextHistory = [
                        ...cleanHistory,
                        {
                            ...
                            questions: structuredClone(quest),
                        },
                    ];
        ...
        (jsonString: string) => {
            try {
                ...
                if (!parsed.questions || !Array.isArray(parsed.questions)) {
                    throw new Error(
                        "Tệp JSON không chứa danh sách câu hỏi hợp lệ"
                    );
                }
                ...
                const newQuestions = parsed.questions.map((q: unknown) => {
                    ...
                    const obj = q as Partial<QuizQuestion> & {
                        [k: string]: unknown;
                    };
                    if (obj == null || typeof obj !== "object") {
                        throw new Error(
                            "Một trong các câu hỏi có cấu trúc không hợp lệ"
                        );
                    }
                ...
                setHistory((prev) => {
                    ...
                    return [
                        ...cleanHistory,
                        { metadata: newMetadata, questions: newQuestions },
                    ];
                });
                setHistoryIndex((prev) => prev + 1);
                setIsDirty(true);
                addToast(
                    `Nhập dữ liệu thành công! Đã tải ${newQuestions.length} câu hỏi.`,
                    "success"
                );
            } catch (err) {
                console.error(err);
                addToast(
                    err instanceof Error
                        ? err.message
                        : "Tệp tin JSON không đúng định dạng cấu trúc Quiz",
                    "error"
                );
            }
        },
    ...
    const appendQuestions = useCallback(
        (newQuestions: QuizQuestion[]) => {
            ...
            markDirty();
            addToast(`Đã thêm ${newQuestions.length} câu hỏi từ AI`, "success");
        },
        [markDirty, addToast]
    );
    ...
    const updateQuestion = useCallback(
        (id: string, updates: QuizQuestion) => {
            setQuestions((prev) =>
    ...
    const toggleExpandQuestion = useCallback(
        (qid: string) => {
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            ...
            if (result?.success) {
                addToast(
                    result.message || "Đã lưu quiz thành công!",
                    "success"
                );
                savedSnapshotRef.current = JSON.stringify({
                    metadata,
