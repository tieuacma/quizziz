  ...
  try {
    ...
    const createPostRes = await fetch(`${BASE_URL}/api/forum/posts`, {
      ...
      body: JSON.stringify({
        ...
        content: 'Chứng minh định lý Fermat lớn cho trường hợp n = 3: $x^3 + y^3 = z^3$. Ai có tài liệu hay không?',
        anonymous: false
      })
    })
    ...
    assert(postData.postId !== undefined, `Post created with ID: ${postData.postId}`)
    assert(postData.moderationStatus === 'approved', 'Post status is approved')
    ...
    assert(bannedPostData.moderationStatus === 'blocked', 'Post status is blocked')
    assert(bannedPostData.message.includes('bị chặn'), 'Message informs about block')

    // ----------------------------------------------------
    // TEST 3: POST CREATION (PENDING due to sensitive word)
    // ----------------------------------------------------
    console.log('\n--- Test 3: Create post with sensitive keywords (Pending) ---')
    const createSensitivePostRes = await fetch(`${BASE_URL}/api/forum/posts`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Cookie': getCookie(student1)
      ...
      body: JSON.stringify({
        ...
        content: 'Cho mình xin đáp án thi cuối kì 2 môn toán lớp 10 với.',
        anonymous: false
      })
    })
    
    assert(createSensitivePostRes.ok, 'Create sensitive post HTTP status is OK')
    const sensitivePostData = await createSensitivePostRes.json()
    assert(sensitivePostData.moderationStatus === 'pending', 'Post status is pending')
    ...
    assert(s1HasPending, 'Author (Student 1) can see their own pending post')
    ...
    assert(!s2HasPending, 'Other students (Student 2) cannot see pending posts')
    ...
    const guestHasPending = listGuest.items.some(p => p.id === pendingPostId)
    ...
    const createCommentRes = await fetch(`${BASE_URL}/api/forum/posts/${normalPostId}/comments`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({
        content: 'Tôi có cách chứng minh bằng cách xét mô-đun dư $x^3 \\equiv y^3 \\pmod 9$.',
        anonymous: false
      })
    })
    ...
    assert(commentData.moderationStatus === 'approved', 'Comment is approved')
    const approvedCommentId = commentData.commentId

    // Add nested comment
    const createNestedRes = await fetch(`${BASE_URL}/api/forum/posts/${normalPostId}/comments`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Cookie': getCookie(student1)
    ...
    const createSensitiveCommentRes = await fetch(`${BASE_URL}/api/forum/posts/${normalPostId}/comments`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Cookie': getCookie(student2)
    ...
    assert(sensitiveCommentData.moderationStatus === 'pending', 'Sensitive comment status is pending')
    ...
    const postDetailsRes = await fetch(`${BASE_URL}/api/forum/posts/${normalPostId}`, {
      headers: { 'Cookie': getCookie(student1) }
    })
    ...
    const nestedComment = details.comments.find(c => c.parentCommentId === approvedCommentId)
    assert(nestedComment !== undefined, 'Nested comment fetched')
    assert(nestedComment.authorName === 'Nguyễn Văn A', 'Author themselves can see their real name on their anonymous comment')

    // Fetch as Student 2 (should see "Học sinh ẩn danh")
    const postDetailsS2Res = await fetch(`${BASE_URL}/api/forum/posts/${normalPostId}`, {
      headers: { 'Cookie': getCookie(student2) }
    })
    const detailsS2 = await postDetailsS2Res.json()
    const nestedCommentS2 = detailsS2.comments.find(c => c.parentCommentId === approvedCommentId)
    assert(nestedCommentS2.authorName === 'Học sinh ẩn danh', 'Other students see "Học sinh ẩn danh" for anonymous comments')
    ...
    assert(vote1.ok === true && vote1.newScore === 1 && vote1.userVote === 1, 'Upvote creates +1 score and sets userVote = 1')
    ...
    assert(vote2.ok === true && vote2.newScore === 0 && vote2.userVote === 0, 'Clicking upvote again retracts vote (score 0, userVote = 0)')
    ...
    assert(vote3.ok === true && vote3.newScore === -1 && vote3.userVote === -1, 'Downvote sets score to -1 and userVote = -1')
    ...
    assert(vote4.ok === true && vote4.newScore === 1 && vote4.userVote === 1, 'Changing downvote to upvote changes score by +2 (score 1, userVote = 1)')

    // ----------------------------------------------------
    // TEST 8: BEST ANSWER SYSTEM
    // ----------------------------------------------------
    console.log('\n--- Test 8: Mark/Toggle Best Answer ---')
    const bestAnswerRes = await fetch(`${BASE_URL}/api/forum/posts/${normalPostId}/best-answer`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    assert(bestAnswerData.bestAnswer.acceptedBy === 'author', 'Accepted by author')
    ...
    assert(report2.error.includes('đã gửi báo cáo'), 'Informs user of duplicate report')

    // 9c. Create 2 more dummy students to report the post, making reportCount >= 3
    const dummyS1 = { userId: 'dummy_s1', email: 'd1@zenith.edu.vn', name: 'Dummy 1', role: 'student' }
    const dummyS2 = { userId: 'dummy_s2', email: 'd2@zenith.edu.vn', name: 'Dummy 2', role: 'student' }

    await fetch(`${BASE_URL}/api/forum/reports`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Cookie': getCookie(dummyS1) },
      body: JSON.stringify({ targetType: 'post', targetId: normalPostId, reason: 'harassment' })
    })

    const report3Res = await fetch(`${BASE_URL}/api/forum/reports`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Cookie': getCookie(dummyS2) },
      body: JSON.stringify({ targetType: 'post', targetId: normalPostId, reason: 'other', details: 'Báo cáo thứ 3' })
    })
    const report3 = await report3Res.json()
    assert(report3.statusChanged === true, 'Post moderation status automatically changed to pending due to reportCount >= 3')
    assert(report3.message.includes('tạm ẩn'), 'Message informs about post being hidden')
    ...
    const queueRes = await fetch(`${BASE_URL}/api/forum/moderation/resolve`, {
    ...
    const hasNormalPostInQueue = queue.posts.some(p => p.id === normalPostId)
    const hasPendingPostInQueue = queue.posts.some(p => p.id === pendingPostId)
    const hasPendingCommentInQueue = queue.comments.some(c => c.id === pendingCommentId)
    const hasOpenReportsInQueue = queue.reports.some(r => r.targetId === normalPostId)

    assert(hasNormalPostInQueue, 'Reported post is in moderation queue')
    assert(hasPendingPostInQueue, 'Pending sensitive post is in moderation queue')
    assert(hasPendingCommentInQueue, 'Pending sensitive comment is in moderation queue')
    assert(hasOpenReportsInQueue, 'Open reports list is in moderation queue')

    // 10b. Teacher approves the reported post (keeping it approved)
    const resolvePostRes = await fetch(`${BASE_URL}/api/forum/moderation/resolve`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
    ...
    const queueRes2 = await fetch(`${BASE_URL}/api/forum/moderation/resolve`, {
      headers: { 'Cookie': getCookie(teacher) }
    })
    const queue2 = await queueRes2.json()
    const reportsForNormalPost = queue2.reports.filter(r => r.targetId === normalPostId)
    assert(reportsForNormalPost.length === 0, 'Reports for resolved post are cleared from open queue')

    // 10c. Teacher blocks the pending comment
    const resolveCommentRes = await fetch(`${BASE_URL}/api/forum/moderation/resolve`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Cookie': getCookie(teacher)
    ...
    assert(resolveComment.action === 'blocked', 'Comment blocked successfully')

    console.log(`\n======================================================`)
    console.log(`TEST RESULTS: Passed ${passCount} / ${testCount} assertions.`)
    console.log(`======================================================`)
    
    if (passCount === testCount) {
      console.log('🎉 ALL TESTS PASSED SUCCESSFULLY! The Forum API is fully functional.')
      process.exit(0)
    } else {
