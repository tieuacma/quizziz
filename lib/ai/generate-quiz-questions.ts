...
export function computeAiQuestionTotal(config: Pick<
  AiGenerateConfig,
  | "multipleChoiceCount"
  | "trueFalseCount"
  | "readingPassageCount"
...
function buildUserPrompt(config: AiGenerateConfig): string {
  ...
  if (config.multipleChoiceCount > 0) {
    parts.push(`Sinh ${config.multipleChoiceCount} câu trắc nghiệm nhiều lựa chọn (4 đáp án A/B/C/D).`);
  }
  if (config.trueFalseCount > 0) {
    parts.push(`Sinh ${config.trueFalseCount} câu Đúng/Sai (options "Đ" và "S").`);
  }
...
export function extractJsonArray(raw: string): unknown {
  ...
  try {
    ...
    if (!arrayMatch) {
      throw new Error("AI không trả về mảng JSON hợp lệ. Vui lòng thử lại.");
    }
    try {
      return JSON.parse(arrayMatch[0]);
    } catch {
      throw new Error("Không thể phân tích JSON từ phản hồi AI. Vui lòng thử lại.");
    }
  }
}
...
function validateOption(opt: unknown, label: string): void {
  if (!isRecord(opt) || typeof opt.id !== "string" || typeof opt.text !== "string") {
...
function validateMultipleChoiceItem(item: Record<string, unknown>, label: string): void {
  ...
  parsed.forEach((item, index) => {
    ...
    if (item.type === "reading") {
      ...
      if (!Array.isArray(item.questions) || item.questions.length === 0) {
        throw new Error(`${label}: thiếu câu hỏi con trong bài đọc hiểu.`);
      }
      item.questions.forEach((sub, subIndex) => {
        if (!isRecord(sub)) {
          throw new Error(`${label} - câu con #${subIndex + 1}: cấu trúc không hợp lệ.`);
        }
        if (sub.type !== "multiple-choice") {
          throw new Error(`${label} - câu con #${subIndex + 1}: phải là multiple-choice.`);
        }
        validateMultipleChoiceItem(sub, `${label} - câu con #${subIndex + 1}`);
      });
      return;
    }
    throw new Error(`${label}: type không được hỗ trợ (${String(item.type)}).`);
  });
}

function normalizeOptions(
  options: Array<{ id: string; text: string }>,
): { options: Array<{ id: string; text: string }>; correctOptionId: string; oldToNew: Map<string, string> } {
...
function normalizeMultipleChoice(
  ...
  const rawOptions = (item.options as Array<{ id: string; text: string }>).map((o) => ({
    id: String(o.id),
    text: String(o.text),
  }));
...
function normalizeReadingSubQuestion(sub: Record<string, unknown>): ReadingSubQuestion {
  const rawOptions = (sub.options as Array<{ id: string; text: string }>).map((o) => ({
    id: String(o.id),
    text: String(o.text),
  }));
  const { options, oldToNew } = normalizeOptions(rawOptions);
  ...
  return parsed.map((item) => {
    ...
    if (item.type === "reading") {
      const subQuestions = (item.questions as unknown[]).map((sub) => {
        if (!isRecord(sub)) throw new Error("Câu hỏi con reading không hợp lệ.");
...
function getResponseContent(response: unknown): string {
  ...
  if (Array.isArray(content)) {
    ...
    if (textPart && isRecord(textPart) && typeof textPart.text === "string") {
...
export async function generateQuizQuestions(
  ...
  if (total > MAX_AI_QUESTIONS) {
    throw new Error(`Tổng số câu hỏi không được vượt quá ${MAX_AI_QUESTIONS}.`);
  }
