...
export function isFillBlankCorrect(
	...
	const current = question.caseSensitive ? answer.trim() : normalizeString(answer);
...
export function isReadingSubQuestionCorrect(
	...
	switch (subQuestion.type) {
		...
		case "fill-in-the-blank": {
			if (typeof answer !== "string" || !Array.isArray(subQuestion.answers)) return false;
			return subQuestion.answers.map(normalizeString).includes(normalizeString(answer));
		}
		default:
			return false;
	}
