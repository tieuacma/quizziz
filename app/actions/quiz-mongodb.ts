...
export type QuizLoadResult =
    { success: true; data: ClientQuiz } | { success: false; error: string };
...
function parseExamTimeLimitSeconds(value: FormDataEntryValue | null): number {
    const raw =
        typeof value === "string" ? Number.parseInt(value, 10) : Number.NaN;
    if (!Number.isFinite(raw) || raw <= 0)
        return DEFAULT_EXAM_TIME_LIMIT_SECONDS;
    return raw;
}
...
export async function createQuizMongoAction(
    ...
    try {
        ...
        const examTimeLimit = parseExamTimeLimitSeconds(
            formData.get("examTimeLimit")
        );
    ...
    try {
        ...
        const quizDoc = {
            ...
            examTimeLimit: (examTimeLimit ??
                (flat.examTimeLimit as number) ??
                DEFAULT_EXAM_TIME_LIMIT_SECONDS) as number,
            questions,
