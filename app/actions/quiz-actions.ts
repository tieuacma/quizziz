...
export async function updateQuestionAction(
  ...
  try {
    ...
    if (!quizId) {
      return { success: false, error: 'ID quiz không hợp lệ.', field: 'quizId' }
    }
    if (!questionId) {
      return { success: false, error: 'ID câu hỏi không hợp lệ.', field: 'questionId' }
    }
    if (!questionJson) {
      return { success: false, error: 'Dữ liệu câu hỏi trống.', field: 'question' }
    }
    ...
    const { updateQuestionInQuizFile } = await import('../../src/lib/quiz-service')
    const result = await updateQuestionInQuizFile(quizId, questionId, parsed)
    ...
    if (!result.success) {
      return { success: false, error: result.error || 'Không thể cập nhật câu hỏi.' }
    }
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Lỗi khi cập nhật câu hỏi.',
    }
  }
}
