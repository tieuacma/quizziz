...
export default async function PostDetailPage({ params }: PostPageProps) {
    ...
    if (
        postDoc.moderationStatus !== "approved" &&
        !isAuthor &&
        !isTeacherOrAdmin
    ) {
        notFound();
    ...
    const mappedPost = {
        ...
        authorName: showPostIdentity
            ? (postDoc.authorName ?? "Thành viên")
            : "Học sinh ẩn danh",
        ...
        content: {
            ...
            attachments: (postDoc.content?.attachments ?? []) as Array<{
                name: string;
                url: string;
                size: number;
                mimeType: string;
        ...
        bestAnswer: postDoc.bestAnswer
            ? {
                  ...
                  acceptedBy: postDoc.bestAnswer.acceptedBy as
                      "teacher" | "author",
    ...
    const mappedComments = commentDocs.map((c) => {
        ...
        const showCommentIdentity =
            !c.anonymous || isCommentAuthor || isTeacherOrAdmin;

        return {
            id: c._id.toString(),
            postId: c.postId.toString(),
            authorId: showCommentIdentity ? c.authorId : "",
            authorName: showCommentIdentity
            ...
            parentCommentId: c.parentCommentId
                ...
                : null,
            ancestorCommentId: c.ancestorCommentId
                ? c.ancestorCommentId.toString()
                : null,
    ...
    const votes = await db
        ...
        .find({
            ...
            $or: [
                ...
                {
                    targetType: "comment",
                    targetId: { $in: commentDocs.map((c) => c._id) },
                },
            ],
        })
        .toArray();
