...
export default async function StudentForumPage({ searchParams }: Props) {
    ...
    const limit = Math.min(
        Math.max(Number(resolvedSearchParams.limit ?? 10), 1),
        50
    );
    ...
    const baseFilter: PostsFilter = {
        deletedAt: null,
        $or: [{ moderationStatus: "approved" }, { authorId: session.userId }],
    };
    ...
    const posts = items.map((doc) => {
        ...
        return {
            ...
            authorName: showIdentity
                ? (doc.authorName ?? "Thành viên")
                : "Học sinh ẩn danh",
        ...
        <div className="space-y-6 max-w-4xl mx-auto pb-10">
            <div>
                ...
                <p className="text-slate-400 text-sm mt-1">
                    Nơi học sinh trao đổi kiến thức, giải đáp các thắc mắc toán
