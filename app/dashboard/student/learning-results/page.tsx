...
export default async function LearningResultsPage() {
  ...
  const onTrack = LEARNING_RESULTS.filter((r) => r.currentAvg >= r.targetAvg).length
      ...
      <StudentSubpageHeader
        ...
        aside={
          <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-5 py-3 text-center sm:text-right min-w-[120px]">
            ...
            <p className="text-2xl font-bold text-emerald-400 mt-0.5">{gpa}</p>
          </div>
        }
      />
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between gap-2 mb-3">
                <h3 className="text-white font-semibold">{r.course}</h3>
                <span
                ...
                >
                  {onTarget ? 'Đạt mục tiêu' : 'Cần cải thiện'}
                </span>
              </div>
              <div className="flex gap-6 text-sm">
                <div>
                  <p className="text-slate-500 text-xs">Hiện tại</p>
                  <p className="text-white font-bold text-lg">{r.currentAvg}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs">Mục tiêu</p>
                  <p className="text-slate-300 font-bold text-lg">{r.targetAvg}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-xs">Tiến độ</p>
                  <p className="text-violet-400 font-bold text-lg">{r.progress}%</p>
                </div>
              </div>
