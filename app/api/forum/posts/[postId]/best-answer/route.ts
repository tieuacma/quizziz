...
export async function POST(req: Request, context: { params: Promise<{ postId: string }> }) {
  ...
  const isTeacherOrAdmin = session.role === 'teacher' || (session.role as string) === 'admin'

  if (!isAuthor && !isTeacherOrAdmin) {
    return NextResponse.json({ error: 'Bạn không có quyền chọn câu trả lời đúng nhất cho bài viết này.' }, { status: 403 })
  }

  // Xác thực bình luận này có thuộc bài viết gốc không
  const commentDoc = await commentsCol.findOne({ _id: _commentId, postId: _postId })
  if (!commentDoc) {
    return NextResponse.json({ error: 'Bình luận không thuộc bài viết này.' }, { status: 400 })
  }
  ...
  if (currentBest && currentBest.commentId.toString() === _commentId.toString()) {
    // Unset
    await postsCol.updateOne({ _id: _postId }, { $unset: { bestAnswer: '' } })
  } else {
    // Set
    newBest = {
    ...
    await postsCol.updateOne({ _id: _postId }, { $set: { bestAnswer: newBest } })
  }
