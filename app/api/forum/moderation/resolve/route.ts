...
export async function POST(req: Request) {
  ...
  const isModerator = session.role === 'teacher' || (session.role as string) === 'admin'
  if (!isModerator) {
  ...
  const reason = String(body.reason ?? 'Được kiểm duyệt thủ công bởi Giáo viên/Admin.').trim()
  ...
  if (targetType === 'post') {
    ...
    await postsCol.updateOne({ _id: targetId }, {
      $set: {
    ...
    if (!commentDoc) return NextResponse.json({ error: 'Bình luận không tồn tại.' }, { status: 404 })

    const oldStatus = commentDoc.moderationStatus

    // Cập nhật trạng thái bình luận
    await commentsCol.updateOne({ _id: targetId }, {
      $set: {
        moderationStatus: action,
        moderationReason: reason,
        updatedAt: new Date()
    ...
    if (action === 'approved' && oldStatus !== 'approved') {
      await postsCol.updateOne({ _id: commentDoc.postId }, { $inc: { 'meta.commentCount': 1 } })
...
// req is required by Next.js route handler signature but not used in body
  ...
  const pendingPosts = await postsCol.find({ moderationStatus: 'pending', deletedAt: null }).toArray()
  const pendingComments = await commentsCol.find({ moderationStatus: 'pending', deletedAt: null }).toArray()

  // Lấy danh sách báo cáo còn mở
  const openReports = await db.collection('reports').find({ status: 'open' }).toArray()
