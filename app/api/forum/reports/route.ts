...
export async function POST(req: Request) {
  ...
  if (targetType === 'post') {
    targetDoc = (await postsCol.findOne({ _id: targetId })) as TargetDoc | null
  } else {
    targetDoc = (await commentsCol.findOne({ _id: targetId })) as TargetDoc | null
  }

  if (!targetDoc) {
    return NextResponse.json({ error: 'Đối tượng bị báo cáo không tồn tại.' }, { status: 404 })
  }
  ...
  if (existingReport) {
    return NextResponse.json({ error: 'Bạn đã gửi báo cáo vi phạm cho nội dung này rồi.' }, { status: 400 })
  }
  ...
  if (reportCount >= 3) {
    ...
    if (targetType === 'post') {
      ...
      if (targetDoc?.moderationStatus === 'approved') {
        ...
        if (targetDoc?.postId) {
          await postsCol.updateOne({ _id: targetDoc.postId }, { $inc: { 'meta.commentCount': -1 } })
        }
