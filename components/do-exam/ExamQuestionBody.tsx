	...
	return (
		<Card className={styles.glassCard}>
			<CardHeader className="flex flex-row items-start justify-between gap-3 pb-2">
				...
				>
					<Flag className={`w-4 h-4 mr-1 ${isFlagged ? "fill-amber-400/30" : ""}`} />
					...
					<div className="space-y-4">
						...
						<div className="space-y-3">
							{question.questions.map((sub, subIndex) => {
								const subAnswer = answer?.subAnswers?.[sub.id] ?? null;
								return (
									<div
										key={sub.id}
									...
									>
										<p className="text-sm text-slate-100 mb-3 font-medium">
											<span className="text-violet-400 mr-1">{subIndex + 1}.</span>
											{sub.question}
										</p>
