			...
			<div className="flex items-start justify-between gap-3">
				...
				>
					{isCritical || isWarning ? (
						<Clock className={cn("w-4 h-4", isCritical ? "text-red-400" : "text-amber-400")} />
					) : (
					...
					<span
						className={cn(
							...
							isCritical ? "text-red-200" : isWarning ? "text-amber-200" : "text-white",
						)}
					>
			...
			<div className="flex flex-col sm:flex-row sm:items-center gap-3">
				<div className="flex-1 min-w-0">
					<div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-wider text-white/50 mb-1">
						...
						<span className="text-emerald-400/90">{answeredCount} đã trả lời</span>
					</div>
					<Progress
						value={progress}
