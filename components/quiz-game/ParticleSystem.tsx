...
export default function ParticleSystem() {
    ...
    const triggerEffect = useCallback(
        (type: ParticleEffectProps["trigger"], x: number, y: number) => {
            const count =
                type === "celebration" ? 50 : type === "streak" ? 30 : 20;
    ...
    useEffect(() => {
        ...
        ).__triggerParticleEffect__ = triggerEffect;
        return () => {
            delete (window as unknown as Record<string, unknown>)
                .__triggerParticleEffect__;
        };
    }, [triggerEffect]);
...
export function useParticleEffect() {
    const triggerEffect = (
        ...
        const globalTrigger = (window as unknown as Record<string, unknown>)
            .__triggerParticleEffect__ as
            | ((
                  type: "correct" | "wrong" | "celebration" | "streak",
                  x: number,
                  y: number
