...
const THEME_OPTIONS = [
  { value: "light" as const, label: "Sáng", icon: <Sun className="h-4 w-4" /> },
  { value: "dark" as const, label: "Tối", icon: <Moon className="h-4 w-4" /> },
  { value: "galaxy" as const, label: "Galaxy", icon: <Sparkles className="h-4 w-4" /> },
];
...
export default function ThemeToggle() {
  ...
  const getActiveIcon = () => {
    ...
    if (theme === "galaxy") return <Sparkles className="h-5 w-5 text-purple-400" />;
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <span className={`transition-colors duration-200 ${
                      active
                        ? theme === "light" ? "text-amber-500" : theme === "galaxy" ? "text-purple-400" : "text-sky-400"
                        : ""
                    }`}>
                      {option.icon}
                    </span>
                  ...
                  {active && <Check className="h-4 w-4 stroke-[3px] text-emerald-500" />}
                </button>
              );
            })}
          </motion.div>
