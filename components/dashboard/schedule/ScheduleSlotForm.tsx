...
export default function ScheduleSlotForm({ schedule, onAdd }: ScheduleSlotFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const nextId =
      schedule.length === 0 ? 1 : Math.max(...schedule.map((item) => item.id)) + 1;
        ...
        <form onSubmit={handleSubmit} className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <div className="space-y-1.5">
            ...
            <Select
              value={form.day}
              onValueChange={(value) =>
                setForm((prev) => ({ ...prev, day: value as WeekDay }))
              }
            >
              <SelectTrigger id="schedule-day" className="border-white/10 bg-white/[0.02]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {WEEK_DAYS.map((day) => (
          ...
          <div className="space-y-1.5 sm:col-span-2 lg:col-span-2">
            <Label htmlFor="schedule-lesson">Nội dung bài học</Label>
            <Input
              id="schedule-lesson"
              placeholder="VD: Routing trong Next.js"
              value={form.lessonTitle}
              onChange={(e) => setForm((prev) => ({ ...prev, lessonTitle: e.target.value }))}
              className="border-white/10 bg-white/[0.02]"
              required
            />
          </div>
          ...
          <div className="flex items-end sm:col-span-2 lg:col-span-1">
            <Button
              ...
              disabled={!form.className.trim() || !form.lessonTitle.trim()}
            >
              Thêm tiết
            </Button>
          </div>
