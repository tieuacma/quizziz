...
import { Send, HelpCircle, Paperclip, Sigma, RefreshCw, CheckCircle2 } from "lucide-react";
...
export default function PostComposer({
  ...
  function handleAddMockFile() {
    const mockFiles = [
      { name: "Đề-cương-ôn-tập-Toán.pdf", url: "https://example.com/doc1.pdf", size: 1024 * 1024 * 1.5, mimeType: "application/pdf" },
      { name: "Sơ-đồ-tư-duy-Hóa-học.png", url: "https://example.com/img2.png", size: 1024 * 512, mimeType: "image/png" }
    ];
    ...
    <div className="relative border border-white/8 bg-gradient-to-b from-white/[0.04] to-white/[0.01] rounded-2xl p-5 shadow-xl overflow-hidden group">
      ...
      <div className="flex items-center justify-between border-b border-white/6 pb-3 mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-white text-sm font-bold">Đặt câu hỏi thảo luận mới</h3>
        </div>
        <button
          type="button"
      ...
      {showMathHelp && (
        <div className="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-3.5 mb-4 text-xs text-emerald-200/90 leading-relaxed space-y-1">
          <p className="font-bold mb-1">Cách chèn công thức Toán học (LaTeX):</p>
          <p>• Dùng <b>$công thức$</b> để viết công thức ngay trong dòng: ví dụ $E=mc^2$ viết là <code className="bg-black/40 px-1 py-0.5 rounded text-emerald-300">$E=mc^2$</code>.</p>
          <p>• Dùng <b>$$công thức$$</b> để tạo khối công thức riêng biệt ở dòng mới: ví dụ <code className="bg-black/40 px-1 py-0.5 rounded text-emerald-300">{'$$\\sum_{1}^{n} i$$'}</code>.</p>
        </div>
      )}

      <div className="space-y-4">
        <label className="block">
          <div className="text-slate-300 text-xs font-semibold mb-1.5">Tiêu đề thảo luận</div>
          <input
        ...
        <label className="block">
          <div className="text-slate-300 text-xs font-semibold mb-1.5">Nội dung câu hỏi</div>
          <textarea
        ...
        {hasMath && content.trim().length > 0 && (
          <div className="bg-white/[0.02] border border-white/6 rounded-xl p-3.5">
            <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-2">Xem trước công thức (Live Preview):</div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-slate-300 text-xs font-semibold">Tệp đính kèm ({attachments.length})</span>
            <button
              type="button"
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-t border-white/6 pt-4 mt-2">
          <label className="flex items-center gap-2.5 text-slate-300 text-xs cursor-pointer select-none py-1 hover:text-white transition">
            ...
            <div className="leading-tight">
              <span className="font-semibold block">Đăng ẩn danh</span>
              <span className="text-[10px] text-slate-400">Che tên với các học sinh khác</span>
            </div>
          </label>
