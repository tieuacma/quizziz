...
export default function PostInteractiveDetail({
    ...
    const [userVotes, setUserVotes] =
        useState<Record<string, number>>(initialUserVotes);
    ...
    async function handleVote(
        ...
        if (targetType === "post") {
            setPost((prev) => ({
                ...
                meta: {
                    ...prev.meta,
                    voteScore: prev.meta.voteScore + scoreDiff,
                },
            }));
        } else {
            setComments((prev) =>
                prev.map((c) =>
                    c.id === targetId
                        ? { ...c, voteScore: c.voteScore + scoreDiff }
                        : c
                )
            );
        }
        ...
        try {
            ...
            if (targetType === "post") {
                setPost((prev) => ({
                    ...
                    meta: {
                        ...prev.meta,
                        voteScore: prev.meta.voteScore + revertDiff,
                    },
                }));
            } else {
                setComments((prev) =>
                    prev.map((c) =>
                        c.id === targetId
                            ? { ...c, voteScore: c.voteScore + revertDiff }
                            : c
                    )
                );
            }
    ...
    async function handleMarkBestAnswer(commentId: string) {
        try {
            ...
            if (!res.ok) {
                setAlertMessage(
                    data.error || "Không thể thiết lập câu trả lời đúng."
                );
                return;
            }
    ...
    function renderComment(node: CommentNode, depth: number = 0) {
        ...
        return (
            <div key={node.id} className={`space-y-3 pt-2 ${indentClass}`}>
                <div
                    className={`border border-white/8 rounded-xl p-4 bg-white/[0.01] hover:bg-white/[0.03] transition relative group ${
                ...
                >
                    ...
                    <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <span
                                    className={`text-xs font-semibold ${node.anonymous ? "text-slate-400 italic" : "text-slate-200"}`}
                                >
                                    {node.authorName}
                                </span>
                    ...
                    <div className="flex items-center justify-between mt-4 pt-2.5 border-t border-white/5">
                        ...
                        <div className="flex items-center gap-1 bg-black/10 border border-white/5 rounded-lg px-1.5 py-0.5">
                            ...
                            <button
                                onClick={() =>
                                    handleVote("comment", node.id, "down")
                                }
                        ...
                        <div className="flex items-center gap-3 text-xs text-slate-400">
                            <button
                                onClick={() =>
                                    setReplyingTo(
                                        replyingTo === node.id ? null : node.id
                                    )
                                }
            ...
            <div className="border border-white/8 bg-white/[0.01] rounded-2xl p-5 sm:p-6 shadow-lg space-y-4">
                ...
                {comments.length === 0 ? (
                    <div className="text-center py-8 text-slate-400 text-sm border border-dashed border-white/6 rounded-xl">
                        Chưa có bình luận nào dưới chủ đề này. Hãy là người trả
                        lời đầu tiên!
                    </div>
                ) : (
