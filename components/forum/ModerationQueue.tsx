...
export default function ModerationQueue() {
    ...
    async function handleAction(
        ...
        if (action === "block") {
            const input = prompt(
                "Nhập lý do khóa/từ chối nội dung này (tùy chọn):"
            );
        ...
        startTransition(async () => {
            try {
                ...
                if (!res.ok) {
                    setActionError(
                        data.error || "Thao tác kiểm duyệt thất bại."
                    );
                    return;
                }

                setActionSuccess(
                    `Đã xử lý thành công: ${action === "approve" ? "Phê duyệt" : "Khóa"}`
                );
                    ...
                    {activeTab === "posts" &&
                        (posts.length === 0 ? (
                            ...
                            posts.map((p) => (
                                <div
                                    key={p.id}
                                    className="border border-white/8 bg-white/[0.02] rounded-2xl p-5 space-y-3 relative group"
                                >
                    ...
                    {activeTab === "reports" &&
                        (reports.length === 0 ? (
                            ...
                            reports.map((r) => (
                                <div
                                    key={r.id}
                                    className="border border-white/8 bg-red-500/5 rounded-2xl p-5 space-y-3 relative group"
                                >
                                    <div className="flex items-start justify-between gap-3">
                                        <div className="space-y-0.5">
